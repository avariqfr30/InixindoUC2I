...
class ProtectedIndonesianEditor:
    ...
    def local_template_issues(text):
        ...
        if sum(lowered.count(label) for label in repeated_labels) >= 3:
            issues.append("repeated_report_frame")
        sentences = [
        ...
        stock_terms = re.findall(
            r"(?i)\b(perlu|dapat|konteks|fokus|status kesiapan|implikasi|intervensi|"
            r"perlu diperkuat|perlu dipastikan|dapat menjadi)\b",
            source,
        )
        if len(stock_terms) >= max(10, len(sentences) // 3):
            issues.append("stock_business_phrase_density")
        return sorted(set(issues + assess_feedback_style(source).get("findings", [])))
    ...
    def polish(self, text, guidance=""):
        ...
        prompt = (
            ...
            f"PANDUAN GAYA KHUSUS USE CASE:\n{str(guidance or '').strip() or 'Gunakan suara analis manajemen yang netral dan berbasis bukti.'}\n\n"
